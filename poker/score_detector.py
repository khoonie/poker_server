...
class Cards:
    ...
    def _group_by_ranks(self) -> Dict[int, List[Card]]:
        # group card by rank
        # return a dict keyed by rank and valued by list of cards with same rank
        # each list is sorted by card values in descending order
    ...
    def _x_sorted_list(self, x) -> List[List[Card]]:
        """
        x = 2 returns a list of pairs, x = 3 returns a list of triples, etc.
        list sorted by sublist ranks
        x = 2 and with a pair of J's and a pair of K's, pair of K's will be the first element of the list
        
        every sublist is a sorted by card suit
        x = 4 and there's a quad of A's, then quad will be sorted by suit - heard, diamond, etc.
