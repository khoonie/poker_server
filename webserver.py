...
def poker_game(ws: WebSocket, connection_channel: str):
    ...
    try:
        ...
        app.logger.error("Unable to connect player {} to a poker server: {}".format(player_id, e.args[0]))

    else:
        client_channel.send_message(server_channel._connection_message)
        ...
        try:
            client_channel.send_message({"message+type": "disconnect"})
        except:
            pass
        finally:
            client_channel.close()
